...
const personalInfo = {
  ...
  hobbies: ["Dancing", "Hiking", "Fishing", "Coding", "Singing", "Swimming"],
  placesLived: [
    {
      place: "Elsinore, Utah",
      length: "3 Years",
...
function dtTemplate(item) {
  return `<dt>${item.place}</dt><dd>${item.length}</dd>`;
}

// function listTemplate(start, item, end) {
...
// function renderdt(selector, list) {
//   const htmlArray = list.map(dtTemplate);
//   document.querySelector(selector).innerHTML = htmlArray.join("");
// }

renderList("#favorite-foods", personalInfo.favoriteFoods, listTemplate);
renderList("#hobbies", personalInfo.hobbies, listTemplate);
renderList("#places-lived", personalInfo.placesLived, dtTemplate);
